...
**Summary: ** A total of `r count_and_percent(bns$q16, "Public Health, Social Services, Social Science, and Education")` students were majoring in a Public Health, Social Services, Social Science, or Education program, while `r count_and_percent(bns$q16, "Science, Technology, Engineering and Math (STEM)")` students majored in `r names(sort(table(bns$q16), decreasing=TRUE)[2])`. 
...
**Summary: ** Student were enrolled in an average of `r round(mean(bns$q18, na.rm = TRUE), 2)` units, while the median was `r median(bns$q18, na.rm = TRUE)`. A total of `r get_count_and_percent(bns$full_time, "Yes")` students were considered full-time (12 or more units), with `r names(sort(table(bns$q18), decreasing=TRUE)[1])` units being the most common response and `r names(sort(table(bns$q18), decreasing=TRUE)[2])` units being the second most common.
