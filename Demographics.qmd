...
#| fig-height: 4
#| fig-width: 6
#| fig-cap: !expr sprintf("%s Reported Number of Dependents %s.", NextFigure(), print_n_reporting('q11'))

tmp$q11 <- as.numeric(tmp$q11)

gghistogram(tmp, x = "q11", add = "mean", alpha = 1, 
            fill = plot_color) + 
  xlab("Age (years)") + ylab("Number of Students") + ylim(0, 3)

# plot_frq(tmp$q11, geom.colors = plot_color, drop.empty = FALSE) + xlab('Number of Dependents') + ylab("Number of Students") + ylim(0, 1.9*max(table(tmp$q11)))
```

**Survey Question: ** *If you have dependents, then how many of them receive more than 50% of their financial support from you?*

**Additional Notes: ** A total of `r get_count_and_percent(bns$q11, "I don't have dependents")` students reporting no dependents and `r get_count_and_percent(bns$q11, "0")` reporting dependents with less than 50% financial support were omitted from the chart. 


**Summary: ** A total of `r sum(table(tmp$q11))` students reported having dependents who receive more than 50% of their financial support. Of those with dependents, `r get_count_and_percent(tmp$q11, '1')` had one dependent, `r get_count_and_percent(tmp$q11, '2')` had two, and `r get_count_and_percent3(tmp$q11, '3')` students had 3 or more dependents.
