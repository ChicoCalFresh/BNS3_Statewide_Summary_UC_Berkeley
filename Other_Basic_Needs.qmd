...
plot_frq(bns$q54, geom.colors = plot_color, wrap.labels = 12) + 
  xlab('\nHow often do you normally use the food pantry?')+ylim(0, 1.3*max(table(bns$q54))) + ylab("Number of Students")
```
...
**Summary:** The most common reason students reported for not using their campus food pantry was because they did not need to use the food pantry, where `r get_count_and_percent(bns$no_pantry_dont_need, "1")` students selected this option. Meanwhile, `r get_count_and_percent(bns$no_pantry_other_students_need, "1")` students felt that other students need the food more than them, and `r get_count_and_percent(bns$no_pantry_not_aware, "1")` reported they were not aware of any campus food pantries or local food banks. Please note, some campuses may not have food pantries on or near their campus. 
